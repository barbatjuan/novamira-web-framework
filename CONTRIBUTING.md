...
| `RT_ORCH_NO_GALLERY_STEP` | FAIL | a router agent (one carrying `## Routing map`) names no gallery-consultation step anywhere in its body — `web-templates` could commit to an archetype before anyone had looked at what the catalogue gallery already ships. Checked on the whole agent document, the same presence shape as `RT_QA_NO_AXIS_CHECK`, so the step may legitimately live in `## Routing map` or `## Order that works` rather than one exact heading |
...
| `RT_TPL_NO_ENVOLTORIO` | WARN (this slice) | a home `TPL-*.md` — a `corporate/` or `ecommerce/` archetype, never a `pages/<role>/` inner page — declares no table with a header cell reading, trimmed and case-insensitively, exactly `Envoltorio`, found the same way `gallery_register_count()` (`:2478`) finds the Registers table: by that literal header cell, never a heading, because the table sits inside `## 2. Wireframe` with no heading of its own on any of the seven archetypes that already carry one. **Two exclusive causes, one row id:** no such table at all, or a table whose SECOND non-`COMP-*` row makes the catch-all ambiguous — `| El resto | contenido | |` is the shape six of the seven compliant archetypes end their table with, so a detector demanding a `COMP-*` id on every row would FAIL the exact convention `catalog-wrapper-integrity`'s Purpose says already complies; one such row is read as the catch-all (key `'*'`), a second means the table stopped saying which default an unlisted section gets. **WARN, not FAIL, in this slice**: sixteen of the twenty-three home archetypes carry no table yet, and their `Envoltorio` tables are Phase-2/3 work, not written today — ratchets to FAIL only once every surviving archetype states one. `TPL-C-14-ritual-bono.md` is this row's own acceptance test: unmodified, it must never fire |
| `RT_TPL_WRAPPER_DUPLICATE` | FAIL | two home archetypes of one family (`recommender.md` § 0 bifurcates on site type, so cross-family is never compared, and `pages/*` carries no Envoltorio contract to compare) declare the IDENTICAL ordered sequence of normalized Envoltorio shapes — `bleed` (a value containing "banda"), `row` ("fila"), or `contained` (everything else, `sec_open()`'s own default, `_build-gallery.php:15468`), compared on the normalized shape and never the raw phrasing, so `"banda a sangre"` and `"banda con fotografía al fondo"` collide on purpose. This is the gap `RT_TPL_TOO_SIMILAR` leaves open: that row diffs section INVENTORY and has no opinion on markup nesting, so two archetypes with different sections could still wrap every one of them the same way and read as the same page with a different accent — precisely the collision `TPL-C-14`'s own table prose exists to name. Ships at FAIL from the moment it lands, no ratchet: unlike its sibling above, it only ever compares files whose Envoltorio table already parsed cleanly, so it cannot be red over a table nobody has written yet. A file that fired `RT_TPL_NO_ENVOLTORIO` is reported by that row and EXCLUDED here — folding sixteen tableless archetypes into one identical all-`contained` signature would bury any real duplicate under them |
| `RT_TPL_ENVOLTORIO_RENDER_MISMATCH` | FAIL | `catalog-wrapper-integrity`'s OTHER half: `RT_TPL_NO_ENVOLTORIO` and `RT_TPL_WRAPPER_DUPLICATE` above only ever read the DOCUMENT; neither opens the generated gallery. `TPL-C-15`'s own `hero_cartera_html()` used to hand-write its `<section>` tag instead of calling `sec_open( …, 'bleed' )`, so its Envoltorio table said "banda a sangre" while the build said "contained", and both existing rows stayed silent because neither one reads built HTML. This row does: for every home `TPL-*.md` whose Envoltorio table parses cleanly, it counts how many `bleedband` and `secrow` sections the archetype's DEFAULT demo actually renders on its home page (`data-arch` on the gallery's own `.sample` div, `data-page="home"`) and compares those two totals against how many `COMP-*` rows in the table declare `bleed`/`row`. **Counted, not positionally matched:** the generator marks no `<section>` with which `COMP-*` it renders, so a 1:1 walk would invent a mapping the generator itself never declares; totals survive without one. **A row whose own cell text names its condition is excluded from the declared count** — `COMP-MAP-SEARCH`'s cell reads "banda a sangre CUANDO el conmutador está en on", `TPL-C-15` ships that toggle `off` by default, and under the default render the section does not exist at all, so counting it as an unconditional `bleed` would FAIL every archetype whose optional band ships off by default. Verified against real bytes: `TPL-C-14` (lumiere) declares 3 bleed + 1 row and renders 3 + 1; `TPL-E-07` (bajura) declares 1 bleed and renders 1; both pass unmodified. `TPL-C-15` (delao) was this row's own regression: it declared 1 unconditional bleed + 1 row and rendered 0 + 1 until `hero_cartera_html()` was fixed |
| `RT_RECOMMENDER_NO_LANE_FORK` | FAIL | `recommender.md`'s `## Flujo` section declares no "sin coincidencia → bespoke" step after step 3 (RECOMENDACIÓN) — a brief matching no `TPL-*` has nowhere documented to go. `template-lane-contract`'s Two-Lane Resolution requirement. Gated on `recommender.md`'s own presence in the audited root, exactly like `RT_GALLERY_NOT_BUILT`/`RT_GALLERY_STALE` gate on the generator's/fingerprint's presence |
| `RT_RECOMMENDER_PROMOTION_GATE_MISSING` | FAIL | `recommender.md`/`web-templates/SKILL.md` names no promotion criterion for a bespoke design — the word alone (`promo…`) is a slogan and a bare `RT_TPL_*` row id alone could be any other rule, so both must be named, in either file. `template-lane-contract`'s Promotion Gate Is Strict requirement. Same presence gate as its sibling above |
...
| `RT_GALLERY_NO_CHARSET` | FAIL | `assets/gallery/index.html` declares no charset within its **first 1024 bytes** — the exact window the HTML spec's encoding-sniffing prescan reads before it renders anything, so a declaration sitting past that offset is as absent as none at all. Found by eye in the published artifact, not by any row that existed before it: the generated bytes are UTF-8 and nothing said so, so a browser fell back to a locale default (windows-1252) and every accented character rendered as a two-character garble, an arrow among them — while this audit stayed green the whole time, because every other row here reasons about structure and none of them ever asked what encoding the bytes on disk claim to be. The check measures `substr(` the output's first 1024 bytes `, 0, 1024 )` for a `<meta charset>`, so a mere `strpos()` for the string "charset" anywhere in the file would not catch a declaration placed too late. Gated on the generator's and the output's presence, exactly like `RT_GALLERY_STALE`'s first two conditions (no fingerprint library needed here), so it never fires in a fixture root that writes only a bare HTML fragment with no real `<head>`. Fix: `php skills/html-mockup/assets/gallery/_build-gallery.php` |
| `RT_GALLERY_AXIS_LEAK` | FAIL | a catalogue gallery strip renders no `data-brand` attribute. `_build-gallery.php` only ever writes `data-brand` when a strip's `$CONTENT` entry names a non-empty `$BRANDS` key, so an unbranded strip is personality-anchor proof — the shared house content demonstrating the five perceptual anchors — sitting in the catalogue surface rather than under `ux-design-system`'s own. Read from the BUILT gallery's own attribute rather than re-parsed from `$STRIPS`/`$BRANDS` PHP source, matching this file's own stated scope: everything it verifies is about a built site |
| `RT_GALLERY_REGISTER_COUNT_MISMATCH` | FAIL | `_gallery-images.md`'s Registers table declares fewer rows than the gallery renders distinct branded demos (the same `data-brand` count `RT_GALLERY_AXIS_LEAK` reads). `RT_GALLERY_ONE_SHOOT`'s per-shoot cap is `ceil(N/R)`; a register count under the demo count states a looser diversity claim than the catalogue actually carries, so the cap the manifest computes is not the cap a ten-demo catalogue should be held to |
| `RT_GALLERY_SINGLE_PAGE_DEMO` | WARN | a branded demo's strip declares only one distinct `data-page` value across its sample(s) — a home with no inner page. An Envato-grade demo is a complete multi-page site even when Phase 3 delivers its pages one at a time (D5), so a demo declaring only a home is not yet Envato-grade. Transitional at WARN this slice — `TPL-E-09` (medida) is already single-page today, rescued only once `TPL-PDP-05` lands — ratchets to FAIL once every surviving demo carries more than one page |
| `RT_MOCKUP_CONTAINER_FORK` | FAIL | a built `.html` mockup asset declares a `--container-max` literal other than the value `web-templates/references/design-system.md`'s own § Contenedores table states. D3: a design brief reconciles TO the house token rather than forking a new measure, however the brief was phrased. Gated on that table row being locatable at all — a fixture root with no such row never carries this check |
| `RT_GALLERY_ACCENT_TEXT_FAIL` | FAIL | `_build-gallery.php`'s `$BRANDS` block, re-measured independently against the same 4.5:1 text-role bar the generator's own `fail()` calls already enforce at build time (WCAG 2.1, `_build-gallery.php`'s own `srgb_lum()`/`contrast()` formula mirrored rather than imported, so the two can drift and this row still catches it). **Two causes:** an accent measuring below 4.5:1 against its brand's `--c-bg` or `--c-bg-alt`, naming the brand and the measured ratio; or the generator's own `< 7.0` ground-type gate and `< 4.5` accent-as-text gate no longer both present as literals in its source — a drift the re-measurement above cannot see through on its own, since re-measuring against a gate that has moved is measuring against nothing |
