...
| `RT_BUILDER_NO_TOKENS` | FAIL | a builder asset under `skills/elementor-core/assets/` has no `es_tokens()` block a scan can be bounded by — the function is absent, it never closes on a line of its own, or the `end of the visual layer` marker is missing or sits above it. An unbounded region is an unscannable one |
| `RT_BUILDER_HARDCODED_TOKEN` | FAIL | a builder asset types a visual literal (hex, `rgba(`, `cubic-bezier`, or a quoted `typography_font_family`) between the close of `es_tokens()` and the `end of the visual layer` marker, and the message names each one as `file:line → literal`. PHP comments are not scanned, and neither is anything below the END marker: a value that cannot reach the emitted data is not a value the axes have to move. **The glob is `skills/elementor-core/assets/*.php` only** — `es-theme-parts.example.php`, `es-product-single.example.php` and `es-shop-template.example.php` carry 108 more literals and no token block, and widening this glob is Task 4 of the axes plan, not a follow-up |
