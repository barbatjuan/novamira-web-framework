...
//
// KEYED, and every other block stays numeric. `implode()` at the assembly line below reads keys
// or not alike, so `index.html` cannot see this: same bytes, same order. The key exists so a
// SECOND artifact — the client chassis, § CHASSIS near the bottom of this file — can build its
// own stylesheet by excluding exactly this one block and nothing else. `.gal-top`, `.strip` and
// `.axis` below belong to the multi-strip catalogue page; no client site ever renders them.
$css['gallery-chrome'] = <<<'CSS'
...
// catalog page and FAILs it for rendering zero `<section class="strip">` — correct for that page,
// wrong for a single-site chassis, which can never have strips. `assets/chassis/` sits beside
