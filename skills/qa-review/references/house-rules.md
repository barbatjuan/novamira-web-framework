...
| 17 | No live orphans, and old URLs go somewhere | A slug that moved leaves no second page competing for it, and its old URL redirects instead of 404ing | Read the `es_slug_redirects` option — `{old_slug: new_slug}`, written by `es_migrate_slug()`. For each pair: (a) confirm exactly ONE page answers the new slug, and that NO published page still sits at the old one; a live page at both is the orphan this row exists for, and it is usually the one search results still point at. (b) Request `/<old>/` server-side WITHOUT following redirects and read the status: `301` is a PASS, `404` and `200` are both FAILs — a `200` means the orphan is still being served. **Expect the FAIL today**: nothing in this framework serves that map, so `es_migrate_slug()` warns on every move and this row is what confirms whether a human or a plugin closed it. An empty/missing option means no slug moved in this build — report N/A, never PASS. Also cross the page inventory against the slugs the build wrote: a published page at a slug the build did not intend is an orphan whether or not it appears in the map | **auto** |
