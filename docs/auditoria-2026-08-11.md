...
| 17, 30 | **Cerrados** por `079a644`: `es_backup_page_state()` respalda el conjunto COMPLETO desplazado (layout, plantilla de página, edit mode, tipo, versión, campos del post y `post_content`) y corre **antes** de la primera escritura —el respaldo anterior estaba cuatro líneas por debajo de las escrituras que debía preceder, así que habría conservado lo que esa misma llamada acababa de escribir; parecía correcto sólo porque `_elementor_data` se escribe la última. Convertir una página clásica avisa. `es_overwrite_preflight()` imprime el bloque que el usuario aprueba, destacando portada, conversión y borrador |
...
| Resto | Abiertos. **Cambios 1 y 2 de ocho, CERRADOS** —los 2 BLOCKER y los 7 MAJOR del camino de escritura. Quedan los cambios 3–8: skill `wordpress-forms`, subagente `novamira-copywriter`, skill `wordpress-legal`, fase de entrega bloqueante, manifiesto de proyecto, filas nuevas de house-rules, arquetipos que faltan y checker de claves de widget |
